...
const BeneficiaryScreen = ({ navigation }) => {
  const [beneficiaries, setBeneficiaries] = useState([{ category: 'Infants', age: '24' }]);
  ...
  return (
    <SafeAreaView style={styles.wrapper}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={[{ backgroundColor: Colors.onPrimaryContainer, padding: 16 + 0 }, styles.row]}>
        ...
        <View style={styles.p16}>
          ...
          {beneficiaries.map((item, i) => (
            <View key={i} style={[styles.mb16, styles.row]}>
              <View style={{ width: '48' + '%' }}>
                <Text variant="bodyMedium" style={[styles.label, { marginBottom: 6 + 0 }]}>
                  Category
                </Text>
                <RNPickerSelect
                  value={item.category}
              ...
              <View style={[{ width: '48' + '%' }, styles.row]}>
                ...
                {i > 0 && (
                  <TouchableOpacity
                    ...
                    <MCIcon name="close" size={24} color={Colors.onPrimaryContainer} />
                  </TouchableOpacity>
                )}
              </View>
            </View>
          ...
          <AppButton mode="text" icon="plus" style={styles.mt16} onPress={addField}>
            Add category
          </AppButton>
        </View>
        <View style={[{ bottom: insets.bottom }, styles.p16, styles.totalBtn]}>
          ...
          <AppButton onPress={() => navigation.navigate('order-summary')}>Total $230</AppButton>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
